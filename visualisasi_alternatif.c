...
void render(SDL_Renderer *renderer, SDL_Texture *texture, Uint32 *pixels) {
    for (int i = 0; i < NX; i++) {
        for (int j = 0; j < NY; j++) {
            float h = eta[i][j] - z[i][j];
            if (h < 0) h = 0;
            int color = (int)(fminf(h * 20000.0f, 255.0f));
            pixels[j * NX + i] = SDL_MapRGB(SDL_AllocFormat(SDL_PIXELFORMAT_ARGB8888), 0, 0, color);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *window = SDL_CreateWindow("Simulasi Banjir", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, NX, NY, 0);
    SDL_Renderer *renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
    SDL_Texture *texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, NX, NY);
    Uint32 *pixels = (Uint32 *)malloc(NX * NY * sizeof(Uint32));

    FILE *logfile = fopen("log_simulasi.csv", "w");
    ...
    for (int step = 0; step < NSTEPS;) {
        ...
        while (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT) step = NSTEPS;
            if (e.type == SDL_KEYDOWN) {
                if (e.key.keysym.sym == SDLK_ESCAPE) step = NSTEPS;
        ...
        float time = step * DT;
        float rain = compute_rainfall(time, rain_pattern);
        simulate_step();
        write_log(logfile, step, time, rain);
